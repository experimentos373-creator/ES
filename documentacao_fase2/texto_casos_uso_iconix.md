...
4. O sistema apresenta o `"Ecrã de Visão Geral"` exibindo as estatísticas com o total de `Equipa`, `Jogo`, `Arbitro`, `Bilhete` vendidos e receita total.

**Caminhos Alternativos:**
4.1 – Sem dados registados na base de dados: O sistema apresenta as estatísticas a zeros no `"Ecrã de Visão Geral"` com a `"Mensagem: Sem dados registados"`.
...
2. O sistema apresenta o `"Formulário de Agendamento"` com os campos de `Jogo`: número, data, hora, `Estadio`, `Equipa` da casa, `Equipa` visitante e fase.
3. O Administrador preenche os campos do `"Formulário de Agendamento"` e clica no botão "Guardar".
4. O sistema valida que o número do `Jogo` não está duplicado, que a data e hora respeitam o formato e que as duas `Equipa` selecionadas não têm `Jogo` agendado no mesmo dia.
...
**Caminhos Alternativos:**
4.1 – Número do `Jogo` já existe: O sistema apresenta a `"Mensagem de Erro: número do Jogo já existe!"` no `"Formulário de Agendamento"` e o caso de uso termina.
...
6. O sistema altera o estado do `Jogo` para finalizado, atribui a vitória ou empate, e atualiza a classificação e os pontos das equipas.
...
5.2 – Empate com Penalties (Eliminatória): O sistema aceita os penalties inseridos pelo Administrador e o fluxo prossegue no passo 6.
...
3. O sistema calcula as estatísticas da `Equipa` contendo o número de `Jogador` do plantel, golos marcados, classificação do grupo e data do próximo `Jogo`.
4. O sistema apresenta o `"Ecrã de Visão Geral da Equipa"` com as estatísticas calculadas.
...
4. O Gestor de Equipa preenche a descrição no campo de nova ocorrência e clica no botão "Adicionar". O sistema regista a ocorrência e atualiza a `"Lista de Histórico de Lesões"`.
...
4. O sistema apresenta o `"Ecrã de Vendas"` exibindo as estatísticas de receita, total de `Bilhete` vendidos e taxa de ocupação.

**Caminhos Alternativos:**
4.1 – Sem bilhetes vendidos no sistema: O sistema exibe as estatísticas a zeros no `"Ecrã de Vendas"` com a `"Mensagem: Sem vendas registadas"`.
...
2. O sistema lê os `Estadio` e os respetivos `SetorEstadio` na base de dados.
3. O sistema apresenta a `"Lista de Inventário"` com nome do `SetorEstadio`, capacidade de público, `Bilhete` emitidos, lugares disponíveis e o preço de cada `Bilhete`.
...
3. O sistema apresenta a `"Tabela de Jogos"` agrupando por fase do campeonato e exibindo data, hora, `Estadio`, as duas `Equipa` concorrentes e o resultado final de cada `Jogo`.
...
2. O sistema calcula a classificação em tempo real para cada grupo ordenando as `Equipa` por pontos, saldo de golos e golos marcados.
...
3. O Adepto escolhe o `SetorEstadio` do `Estadio`, introduz a quantidade de `Bilhete` e clica no botão "Comprar".
4. O sistema valida que a quantidade está entre 1 e 4 (regra anti-bot) e que o `SetorEstadio` possui capacidade restante na base de dados.
5. O sistema desconta a quantidade de lugares disponíveis do `SetorEstadio` e regista a compra na base de dados.
...
4.2 – Lugares insuficientes no `SetorEstadio` do `Estadio`: O sistema impede a transação, apresenta a `"Mensagem de Erro: Setor esgotado"` no `"Ecrã de Compra de Bilhetes"` e o caso de uso termina.
